...
void setRadius(uint8_t valRadius) {
	radius = valRadius;
}
...
bool setFreqPWM(uint16_t frequency)
{
	i2c_device ddev = getDev(device);
...
bool setError(uint8_t deviation)
{
	i2c_device ddev = getDev(device);
...
bool setSpeed(float valSpeed, uint8_t typeSpeed, float valStop, uint8_t typeStop)
{
	i2c_device ddev = getDev(device);
...
bool setStop(float value, uint8_t type)
{
	i2c_device ddev = getDev(device);
...
bool setInvGear(bool invRDR, bool invPIN)
{
	i2c_device ddev = getDev(device);
...
float getSum(uint8_t type)
{						 //	Параметр:				type - тип запрашиваемого значения: MOT_MET / MOT_REV.
...
bool setVoltage(float motVoltage)
{ //	Параметр:				motVoltage - напряжение в В.
	if (valAddr)
	{	//	Если модуль был инициализирован, то ...
		//	Готовим данные:																									//
		if (motVoltage > 25.5)
...
bool setNominalRPM(uint16_t value)
{ //	Параметр:				value - скорость вращения вала редуктора от 0 до 65'535 об/мин.
	if (valAddr)
	{												//	Если модуль был инициализирован, то ...
													//	Готовим данные:																									//
...
bool saveManufacturer(uint64_t code)
{ //	Параметр:				code - код разработчика.
	if (valAddr)
	{	//	Если модуль был инициализирован, то ...
		//	Ждём:																											//
...
{						 //	Параметры:				reg - номер первого регистра, sum - количество записываемых байт, num - номер первого элемента массива data.
	...
	do
	{
		result = i2c_ioctl_write(dev, reg, data, sum);
		sumtry--;
		if (!result)
		{
			delay(1);
