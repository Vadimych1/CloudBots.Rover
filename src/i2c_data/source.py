...
#         self.e = np.array([0.0, 1.0, 0.0])

#         self.sensor.set_accel_range()

#         acc_calibr = []
#         gyro_calibr = []
#         for _ in range(100):
#             acc_calibr.append(self.sensor.get_accel_data())
#             gyro_calibr.append(self.sensor.get_gyro_data())
#             time.sleep(0.06)

#         self.acc_mid = np.array([
#             sum(map(lambda x: x["x"], acc_calibr)) / len(acc_calibr),
#             sum(map(lambda x: x["y"], acc_calibr)) / len(acc_calibr),
#             sum(map(lambda x: x["z"], acc_calibr)) / len(acc_calibr)
#         ])

#         self.gyro_mid = np.array([
#             sum(map(lambda x: x["x"], gyro_calibr)) / len(gyro_calibr),
#             sum(map(lambda x: x["y"], gyro_calibr)) / len(gyro_calibr),
#             sum(map(lambda x: x["z"], gyro_calibr)) / len(gyro_calibr)
#         ])

#     def accelerometer(self):
#         dat = self.sensor.get_accel_data()
#         return np.array([dat["x"], dat["y"], dat["z"]]) - self.acc_mid
    
#     def gyroscope(self):
#         dat = self.sensor.get_gyro_data()
#         return np.array([dat["x"], dat["y"], dat["z"]]) - self.gyro_mid
...
class MPU:
    ...
    def compFilter(self):
        ...
        accPitch = np.degrees(np.atan2(self.ay, self.az))
        accRoll = np.degrees(np.atan2(self.ax, self.az))
